...
The reader protocol's "related active ADR" requirement is scoped by match, not by the full ADR directory. Read superseded ADRs only for the exception cases in `INDEX.md` retrieval policy and ADR lifecycle checks.
...
When no ADR is created, preserve durable context in the record type that owns it: verified implementation facts in `architecture/current.md`; accepted functional logic, invariants, critical scenarios, interfaces, boundaries, and non-goals in the smallest matching `architecture/real_arch/` topic. Add retrieval cues only when they make an approved new or materially changed record easier to find.

When matching active ADRs collectively define a stable model missing from those authority records, propose the smallest `current.md` or `real_arch` synthesis and a helpful cue. Leave ADR history unchanged.
