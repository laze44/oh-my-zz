...
For complete root `AGENTS.md` generation/reinitialization with an identical `CLAUDE.md` and fresh memory, use the `agent-init` skill. It reuses this workflow in `docs-only` mode and separately owns the explicitly requested root-file rewrite. A memory-only request never authorizes that rewrite.
