...
This repository is a focused ten-skill pack for Claude Code, Codex, OpenCode, and Kimi Code. It intentionally contains only:
...
`idea-to-spec-and-plan` owns one complete logical plan with a single entry point, inline requirements by default, an optional independent spec, and supporting fragments when useful. Both planning skills clarify material completion decisions one question at a time and define one or two final outcome tasks with required evidence and constraints; development work items are not user acceptance checkpoints. Required system processing stages, order, and contracts remain plan constraints with proportionate production-path evidence; development autonomy does not authorize changing them. The formal plan's execution policy requests suitable subagent delegation while leaving decomposition and scheduling to the executor; planning itself never starts implementation or reviewers. `systems-paper-writing` owns evidence-grounded planning, drafting, and revision for systems, networking, and computer architecture papers; it does not perform peer review. `paper-review` owns architecture manuscript audits and HPCA-style expert review in two separate reports, using only the paper as evidence unless the author explicitly expands the source scope. `brief-change-plan` creates concise dated plans for bounded changes without a fixed section layout, code, or independent review. `agent-init` owns explicitly requested root instruction regeneration, identical CLAUDE.md copying, and bundled docs-only memory initialization. `project-memory-init` owns fresh Markdown wiki setup and explicitly previewed schema/reader/discovery upgrades. `project-architecture-sync` owns manual post-implementation factual synchronization and separately approved hard-rule updates from sync, conversation, or authorized plan execution. Ordinary agents retrieve unfamiliar concepts before user questions and read applicable global/real_arch hard rules before planning or changing code. Planning records exact separately approved rule changes; execution applies them before dependent code. Ordinary wiki facts may lag implementation and update only on manual sync, with one exact draft reviewed at a time. User-approved hard rules may precede implementation. Both memory workflows may use ephemeral external Stop-gate state; it never becomes project memory. `handoff` is an explicitly user-invoked, temporary-session transfer workflow and does not create durable project records. Do not add other lifecycle skills or platform integrations without an explicit scope decision.

## Intent mapping

- Generate or reinitialize root AGENTS.md, copy identical CLAUDE.md, and bootstrap missing project memory → `agent-init`
...
- Architecture paper review, manuscript issue audits, and reviewer-style revision advice → `paper-review`
